...
import TaskParser.actions.*;
import TaskParser.listeners.TextEditMenuListener;
import TaskParser.listeners.UndoMenuListener;
...
public class MenuHelper {
    ...
    public static JMenuItem addMenuItem(JMenu parent, String text, ActionListener actionListener) {
        JMenuItem jMenuItem = new JMenuItem(text); //создаём новый пункт меню на основе text
        jMenuItem.addActionListener(actionListener); //устанавливаем созданному пункту слушателя, переданного в конструктор
        parent.add(jMenuItem); //добавляем в переданный параметр Parent, созданный пункт меню

        return jMenuItem;//возвращаем созданный пункт меню и выходим из метода
    }

    public static JMenuItem addMenuItem(JMenu parent, Action action) {//метод аналогичен предыдущему но в кач-ве параметра принимает действие action
